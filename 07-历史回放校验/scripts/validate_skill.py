...
LOCAL_REQUIRED_FILES = [
    "SKILL.md",
    "core_manifest.json",
    "schemas/本体历史回放校验输出结构.json",
    "examples/01_历史回放校验样例.json",
]
SKILL_REQUIRED_PHRASES = [
    "touyan-benti-lishi-huifang-xiaoyan",
    "ontology_bundle_vN",
    "target_file",
    "historical_cases",
    "t0",
...
def assert_schema_and_examples():
    schema = load_json(ROOT / "schemas" / "本体历史回放校验输出结构.json")
    jsonschema.Draft202012Validator.check_schema(schema)
    load_json(ROOT / "examples" / "01_历史回放校验样例.json")
...
def main():
    ...
    print("OK: historical validation skill validated")
